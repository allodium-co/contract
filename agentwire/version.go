...
// contract and the compatibility policy that governs skew between the two
// planes, which ship on independent cadences: a customer pins a data-plane
// version and upgrades on their own schedule, so the control plane must
// tolerate older agents on the gRPC stream.
//
...
//     the deprecation process in docs/version-compatibility.md. The
...
// field). It is published alongside the neutral JSON Schema so a BYO
...
// the policy documented in docs/version-compatibility.md.
...
const (
	...
	// the skew (see docs/version-compatibility.md).
	SupportTooOld
)
